        ...
        <form onSubmit={handleAddCategory} className="mb-6">
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              <ShoppingBag className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4" />
        ...
        <div className="space-y-1 mt-2">
          {isLoading ? (
            ...
            categories.map((category) => (
              <div key={category._id} className="flex items-center justify-between p-3 rounded hover:bg-muted">
                <div className="flex items-center">
                  <div className="mr-3 text-muted-foreground">{getCategoryIcon(category.name)}</div>
                  <span>{category.name}</span>
                </div>
                <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-700 hover:bg-red-500/10" onClick={() => setDeleteCategoryId(category._id)}>
